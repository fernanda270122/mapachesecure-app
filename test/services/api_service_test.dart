...
void main() {
  group('Pruebas unitarias para ApiService', () {
    ...
    test(
      '3. GET 500 con detail lanza Exception con el mensaje del servidor',
      () {
        final api = _servicio(
          MockClient(
            (_) async =>
                http.Response(jsonEncode({'detail': 'Error interno'}), 500),
          ),
        );
        expect(
          () => api.get('/algo'),
          throwsA(
            predicate<Exception>((e) => e.toString().contains('Error interno')),
          ),
        );
      },
    );
    ...
    test(
      '5. GET incluye Authorization header cuando hay token en prefs',
      () async {
    ...
    test(
      '6. GET no incluye Authorization header cuando no hay token en prefs',
      () async {
    ...
    test('7. POST 200 retorna el cuerpo decodificado', () async {
      final api = _servicio(
        MockClient(
          (_) async => http.Response(jsonEncode({'guardado': true}), 201),
        ),
      );
    ...
    test('8. POST 400 con detail lanza Exception', () {
      final api = _servicio(
        MockClient(
          (_) async =>
              http.Response(jsonEncode({'detail': 'Datos inválidos'}), 400),
        ),
      );
      expect(
        () => api.post('/actividad', {}),
        throwsA(
          predicate<Exception>((e) => e.toString().contains('Datos inválidos')),
        ),
      );
    });
    ...
    test('10. PUT 200 retorna los datos actualizados', () async {
      final api = _servicio(
        MockClient(
          (_) async => http.Response(jsonEncode({'actualizado': true}), 200),
        ),
      );
    ...
    test('11. PUT 401 lanza ApiUnauthorizedException', () {
      final api = _servicio(MockClient((_) async => http.Response('', 401)));
      expect(
        () => api.put('/usuario/1', {}),
        throwsA(isA<ApiUnauthorizedException>()),
      );
    ...
    test('12. PUT 500 lanza Exception', () {
      final api = _servicio(
        MockClient(
          (_) async =>
              http.Response(jsonEncode({'detail': 'Fallo del servidor'}), 500),
        ),
      );
    ...
    test('13. DELETE 200 retorna confirmación', () async {
      final api = _servicio(
        MockClient(
          (_) async => http.Response(jsonEncode({'eliminado': true}), 200),
        ),
      );
    ...
    test('15. DELETE 500 lanza Exception', () {
      final api = _servicio(
        MockClient(
          (_) async =>
              http.Response(jsonEncode({'detail': 'Error al eliminar'}), 500),
        ),
      );
