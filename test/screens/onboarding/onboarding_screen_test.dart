...
void main() {
  ...
  group('Pruebas para OnboardingScreen', () {
    testWidgets(
      ...
      (tester) async {
        await tester.pumpWidget(
          _wrap(const OnboardingScreen(rol: 'padre', destino: SizedBox())),
        );
        await tester.pump();
    ...
    testWidgets('2. Rol hijo muestra el slide inicial de bienvenida correcto', (
      tester,
    ) async {
      await tester.pumpWidget(
        _wrap(const OnboardingScreen(rol: 'hijo', destino: SizedBox())),
      );
      await tester.pump();
      expect(find.text('¡Hola! Bienvenido a Raccu'), findsOneWidget);
    });

    testWidgets('3. Botón "Saltar" está visible en pantalla', (tester) async {
      await tester.pumpWidget(
        _wrap(const OnboardingScreen(rol: 'padre', destino: SizedBox())),
      );
      await tester.pump();
    ...
    testWidgets('4. Primer slide muestra botón "Siguiente", no "¡Comenzar!"', (
      tester,
    ) async {
      await tester.pumpWidget(
        _wrap(const OnboardingScreen(rol: 'padre', destino: SizedBox())),
    ...
    testWidgets('5. PageView está presente en la pantalla', (tester) async {
      await tester.pumpWidget(
        _wrap(const OnboardingScreen(rol: 'hijo', destino: SizedBox())),
      );
      await tester.pump();
    ...
    testWidgets('7. Tap "Siguiente" avanza de página y dispara onPageChanged', (
      tester,
    ) async {
      await tester.pumpWidget(
        _wrap(const OnboardingScreen(rol: 'padre', destino: SizedBox())),
    ...
    testWidgets('8. Tap "Saltar" llama _terminar() y navega al destino', (
      tester,
    ) async {
      await tester.pumpWidget(
        _wrap(
