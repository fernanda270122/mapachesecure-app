...
void main() {
  ...
  group('Pruebas para DetalleDesafioScreen', () {
    testWidgets('1. Muestra el título del desafío en el AppBar', (
      tester,
    ) async {
    ...
    testWidgets('2. Muestra el badge de dificultad "NIVEL: MEDIO"', (
      tester,
    ) async {
      await tester.pumpWidget(_wrap());
      await tester.pumpAndSettle();
    ...
    testWidgets('3. Muestra la descripción del desafío', (tester) async {
      await tester.pumpWidget(_wrap());
      await tester.pumpAndSettle();
      expect(
    ...
    testWidgets('4. Muestra la recompensa en puntos', (tester) async {
      await tester.pumpWidget(_wrap());
      await tester.pumpAndSettle();
      expect(find.textContaining('50 pts'), findsOneWidget);
    });

    testWidgets('5. Muestra el placeholder para la cámara', (tester) async {
      await tester.pumpWidget(_wrap());
      await tester.pumpAndSettle();
      expect(find.text('Toca para sacar la foto de evidencia'), findsOneWidget);
    });

    testWidgets('6. Muestra el botón "ENVIAR DESAFÍO"', (tester) async {
      await tester.pumpWidget(_wrap());
      await tester.pumpAndSettle();
      expect(find.text('ENVIAR DESAFÍO'), findsOneWidget);
    });

    testWidgets('7. Tap ENVIAR sin foto muestra SnackBar de advertencia', (
      tester,
    ) async {
    ...
    testWidgets('10. Dificultad nula usa texto default "NORMAL" en el badge', (
      tester,
    ) async {
      await tester.pumpWidget(
        _wrap(const {
    ...
    testWidgets('11. Título nulo en desafio usa texto por defecto en AppBar', (
      tester,
    ) async {
      await tester.pumpWidget(
        _wrap(const {
    ...
    testWidgets('12. Tap en el botón de altavoz llama a _hablarInstrucciones', (
      tester,
    ) async {
      await tester.binding.setSurfaceSize(const Size(1080, 1920));
      addTearDown(() => tester.binding.setSurfaceSize(null));
    ...
    testWidgets('13. Tap en zona de cámara llama a _tomarFoto sin crash', (
      tester,
    ) async {
      await tester.binding.setSurfaceSize(const Size(1080, 1920));
      addTearDown(() => tester.binding.setSurfaceSize(null));
    ...
    testWidgets('14. Navegar fuera de la pantalla llama dispose sin crash', (
      tester,
    ) async {
      await tester.pumpWidget(
        ChangeNotifierProvider(
                ...
                body: Builder(
                  builder: (c) => ElevatedButton(
                    onPressed: () => Navigator.push(
                      ...
                      MaterialPageRoute(
                        builder: (_) =>
                            DetalleDesafioScreen(desafio: _desafioTest),
                      ),
                    ),
                    child: const Text('Ir'),
                  ),
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        final tempFile = File(
          '${Directory.systemTemp.path}/test_evidencia.jpg',
        );
