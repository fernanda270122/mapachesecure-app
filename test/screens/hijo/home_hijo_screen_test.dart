...
const _desafioBase =
    '{"id":"1","titulo":"Reto prueba","descripcion":"Descripcion del reto","tipo":"cognitivo","esta_activo":true,"dificultad":"facil","puntos":10}';
...
void main() {
  ...
  group('Pruebas para HomeHijoScreen', () {
    testWidgets('1. Contiene un Scaffold como raíz de la pantalla', (
      tester,
    ) async {
    ...
    testWidgets('2. Muestra el saludo al usuario tras cargar', (tester) async {
      await tester.pumpWidget(_wrap());
      await tester.pump();
      expect(find.textContaining('¡Hola,'), findsOneWidget);
    });

    testWidgets('3. Muestra el ícono de menú hamburguesa', (tester) async {
      await tester.pumpWidget(_wrap());
      await tester.pump();
    ...
    testWidgets('5. Muestra "Desafíos disponibles:" en el cuerpo', (
      tester,
    ) async {
      await tester.pumpWidget(_wrap());
    ...
    testWidgets('6. Muestra mensaje cuando no hay desafíos', (tester) async {
      await tester.pumpWidget(_wrap());
      await tester.pump(const Duration(milliseconds: 100));
      expect(find.text('No hay desafíos disponibles'), findsOneWidget);
    });

    testWidgets('7. Muestra "RaccuPoints" en la tarjeta de mascota', (
      tester,
    ) async {
      await tester.pumpWidget(_wrap());
      await tester.pump(const Duration(milliseconds: 100));
  ...
  group('Pruebas con datos', () {
    ...
    testWidgets('9. Muestra nivel 0 en el badge cuando los puntos son 0', (
      tester,
    ) async {
    ...
    testWidgets('10. Muestra card de desafio cognitivo con tipo COGNITIVA', (
      tester,
    ) async {
    ...
    testWidgets('11. Muestra card de desafio fisico con tipo FISICA', (
      tester,
    ) async {
      await cargar(
        tester,
        desafiosJson:
            '[{"id":"1","titulo":"Reto fisico","descripcion":"Desc","tipo":"fisico","esta_activo":true,"dificultad":"medio","puntos":15}]',
      );
      expect(find.text('FISICA', skipOffstage: false), findsOneWidget);
    });

    testWidgets('12. Muestra card de desafio de orden con icono correcto', (
      tester,
    ) async {
      await cargar(
        tester,
        desafiosJson:
            '[{"id":"1","titulo":"Reto orden","descripcion":"Desc","tipo":"orden","esta_activo":true,"dificultad":"dificil","puntos":20}]',
      );
    ...
    testWidgets('13. Badge de dificultad FACIL se muestra en la card', (
      tester,
    ) async {
      await cargar(tester, desafiosJson: '[$_desafioBase]');
    ...
    testWidgets('14. Badge de dificultad DIFICIL se muestra en la card', (
      tester,
    ) async {
      await cargar(
        tester,
        desafiosJson:
            '[{"id":"1","titulo":"Reto dificil","descripcion":"Desc","tipo":"orden","esta_activo":true,"dificultad":"dificil","puntos":20}]',
      );
      expect(find.text('DIFICIL', skipOffstage: false), findsOneWidget);
    });

    testWidgets('15. Expandir card muestra boton para ir al desafio', (
      tester,
    ) async {
      await cargar(tester, desafiosJson: '[$_desafioBase]');
    ...
    testWidgets('16. Card de desafio pendiente muestra estado de revision', (
      tester,
    ) async {
      await cargar(
        tester,
      ...
      await tester.pump(const Duration(milliseconds: 300));
      expect(
        find.text('Pendiente de revisión', skipOffstage: false),
        findsOneWidget,
      );
    });
    ...
    testWidgets('18. Drawer muestra opciones de navegacion al abrirse', (
      tester,
    ) async {
      await cargar(tester);
      ...
      expect(
        find.text('Tienda de recompensas', skipOffstage: false),
        findsOneWidget,
      );
    });
    ...
    group('Niveles de mascota', () {
      ...
      testWidgets('21. 5500 puntos muestran Nivel 6 y progreso al 100%', (
        tester,
      ) async {
    ...
    group('Dificultad sin reconocer', () {
      testWidgets('22. Dificultad desconocida muestra badge en mayúsculas', (
        tester,
      ) async {
        await cargar(
          tester,
    ...
    group('Escudo de cierre de sesión', () {
      ...
      testWidgets(
        '23. Tap Cerrar Sesión abre diálogo de Validación de Adulto',
        (tester) async {
      ...
      testWidgets('24. CANCELAR en diálogo de cierre de sesión lo cierra', (
        tester,
      ) async {
        await abrirEscudo(tester);
      ...
      testWidgets(
        '25. DESACTIVAR con credenciales inválidas muestra SnackBar de error',
        (tester) async {
          await abrirEscudo(tester);
          ...
          await tester.pump(const Duration(milliseconds: 300));
          expect(
            find.text('Datos incorrectos o acceso denegado.'),
            findsOneWidget,
          );
    ...
    group('Cargar datos: casos extra', () {
      testWidgets(
        '26. tipo_avatar y foto_perfil del backend actualizan el estado',
        (tester) async {
          await tester.binding.setSurfaceSize(const Size(1080, 1920));
          addTearDown(() => tester.binding.setSurfaceSize(null));
          ApiService.testClient = MockClient((req) async {
      ...
      testWidgets('27. Desafio con esta_activo=false no se muestra en pantalla', (
        tester,
      ) async {
        await cargar(
          tester,
        ...
        expect(
          find.text('Desafio Inactivo', skipOffstage: false),
          findsNothing,
        );
        expect(find.text('No hay desafíos disponibles'), findsOneWidget);
      });
      ...
      testWidgets(
        '28. Dos desafios con mismo titulo: solo uno visible por deduplicación',
        (tester) async {
          await cargar(
            tester,
            desafiosJson:
      ...
      testWidgets('29. Desafio de tipo general usa icono por defecto', (
        tester,
      ) async {
        await cargar(
          tester,
      ...
      testWidgets(
        '30. Error de API 500 no bloquea la pantalla y muestra nombre',
        (tester) async {
          ApiService.testClient = MockClient(
      ...
      testWidgets('31. Dificultad "medio" usa color naranja (badge visible)', (
        tester,
      ) async {
        await cargar(
          tester,
      ...
      testWidgets(
        '32. Tap en avatar del header inicia navegación a AvatarScreen',
        (tester) async {
          await cargar(tester);
    ...
    group('Drawer opciones adicionales', () {
      ...
      testWidgets('33. Tap en Tienda de recompensas en drawer navega', (
        tester,
      ) async {
    ...
    group('Escudo de sesión: login exitoso', () {
      ...
      testWidgets('39. Login exitoso con rol=padre navega al LoginScreen', (
        tester,
      ) async {
      ...
      testWidgets('40. Login con rol no-padre lanza SnackBar de error', (
        tester,
      ) async {
        await abrirEscudo(tester);
        ApiService.testClient = MockClient((req) async {
    ...
    group('Cobertura adicional', () {
      ...
      testWidgets('42. Tap en botón Recompensas del cuerpo navega', (
        tester,
      ) async {
        await cargar(tester);
      ...
      testWidgets('52. Volume button visible tras expandir card cubre L1088', (
        tester,
      ) async {
        await cargar(tester, desafiosJson: '[$_desafioBase]');
        await tester.pump(const Duration(milliseconds: 100));
