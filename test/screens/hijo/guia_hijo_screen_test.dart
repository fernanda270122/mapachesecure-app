...
void main() {
  ...
  group('Pruebas para GuiaHijoScreen', () {
    testWidgets('1. Muestra "Guía de la app" en el AppBar', (tester) async {
      await tester.pumpWidget(_wrap());
      await tester.pumpAndSettle();
      expect(find.text('Guía de la app'), findsOneWidget);
    });

    testWidgets('2. Muestra sección "¡Bienvenido a Raccu!"', (tester) async {
      await tester.pumpWidget(_wrap());
      await tester.pumpAndSettle();
      expect(find.text('¡Bienvenido a Raccu!'), findsOneWidget);
    });

    testWidgets('3. Muestra sección sobre los desafíos', (tester) async {
      await tester.pumpWidget(_wrap());
      await tester.pumpAndSettle();
      expect(find.text('¿Cómo funcionan los desafíos?'), findsOneWidget);
    });

    testWidgets('4. Muestra sección sobre los puntos', (tester) async {
      await tester.pumpWidget(_wrap());
      await tester.pumpAndSettle();
      expect(find.text('¿Para qué sirven los puntos?'), findsOneWidget);
    });

    testWidgets('5. Al hacer scroll muestra sección "Tu mascota Raccu"', (
      tester,
    ) async {
      await tester.pumpWidget(_wrap());
      await tester.pumpAndSettle();
      await tester.scrollUntilVisible(find.text('Tu mascota Raccu'), 300.0);
      expect(find.text('Tu mascota Raccu'), findsOneWidget);
    });

    testWidgets('6. Al hacer scroll muestra sección sobre apps bloqueadas', (
      tester,
    ) async {
      await tester.pumpWidget(_wrap());
      await tester.pumpAndSettle();
      ...
      expect(find.text('¿Por qué están bloqueadas mis apps?'), findsOneWidget);
    });
