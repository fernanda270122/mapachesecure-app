...
void main() {
  ...
  group('Pruebas para HomePadreScreen', () {
    testWidgets('1. Muestra "Panel de Control" en el AppBar', (tester) async {
      await tester.pumpWidget(_wrap());
      await tester.pump(const Duration(milliseconds: 200));
      expect(find.text('Panel de Control'), findsOneWidget);
    });

    testWidgets('2. Muestra el ícono del menú hamburguesa (drawer)', (
      tester,
    ) async {
      await tester.pumpWidget(_wrap());
      await tester.pump(const Duration(milliseconds: 200));
    ...
    testWidgets('3. El AppBar tiene foregroundColor blanco', (tester) async {
      await tester.pumpWidget(_wrap());
      await tester.pump(const Duration(milliseconds: 200));
    ...
    testWidgets('5. Muestra "Hijos conectados" en estado cargado', (
      tester,
    ) async {
      await tester.pumpWidget(_wrap());
    ...
    testWidgets('6. Muestra mensaje cuando no hay hijos registrados', (
      tester,
    ) async {
      await tester.pumpWidget(_wrap());
      await _pumpLoaded(tester);
  ...
  group('Pruebas con datos', () {
    ...
    Future<void> cargar(
      ...
      ApiService.testClient = MockClient((req) async {
        ...
        if (path.contains('/puntos'))
          return http.Response('{"total_puntos": 50}', 200);
    ...
    testWidgets('9. Muestra el subtitulo Toca para configurar con un hijo', (
      tester,
    ) async {
      await cargar(tester);
    ...
    testWidgets('10. Muestra la seccion Actividad de hoy con un hijo', (
      tester,
    ) async {
      await cargar(tester);
    ...
    testWidgets('12. Con 2 hijos muestra tarjeta con stats para cada hijo', (
      tester,
    ) async {
    ...
    testWidgets('14. Muestra RefreshIndicator tras cargar datos', (
      tester,
    ) async {
      await cargar(tester);
    ...
    testWidgets('15. Drawer se abre y muestra los items de navegación', (
      tester,
    ) async {
    ...
    testWidgets(
      '17. Resumen muestra formato de horas cuando tiempo es mayor a 60 min',
      (tester) async {
        ApiService.testClient = MockClient((req) async {
          final path = req.url.path;
    ...
    testWidgets('18. Tap en tarjeta de hijo navega a ConfigurarHijoScreen', (
      tester,
    ) async {
      ApiService.testClient = MockClient((req) async {
        final path = req.url.path;
    ...
    testWidgets(
      '19. Error de API en carga activa catch y muestra pantalla vacía',
      (tester) async {
        ApiService.testClient = MockClient((req) async {
    ...
    testWidgets(
      '20. Drawer → Gestionar Desafíos navega a la pantalla correcta',
      (tester) async {
        final prevOnError = FlutterError.onError;
        FlutterError.onError = (d) {
          if (d.exceptionAsString().contains('overflowed')) return;
        ...
        await tester.pump(const Duration(milliseconds: 300));
        expect(
          find.text('Gestionar Desafíos', skipOffstage: false),
          findsWidgets,
        );
      },
    );

    testWidgets('21. Drawer → Agregar Hijo/a navega a AgregarHijoScreen', (
      tester,
    ) async {
      final prevOnError = FlutterError.onError;
      FlutterError.onError = (d) {
    ...
    testWidgets(
      '22. Drawer → Revisar Evidencias navega a la pantalla correcta',
      (tester) async {
        final prevOnError = FlutterError.onError;
        FlutterError.onError = (d) {
          if (d.exceptionAsString().contains('overflowed')) return;
        ...
        await tester.pump(const Duration(milliseconds: 300));
        expect(
          find.text('Revisar Evidencias', skipOffstage: false),
          findsWidgets,
        );
      },
    );
    ...
    testWidgets(
      '23. Drawer → Tienda de Recompensas navega a la pantalla correcta',
      (tester) async {
        final prevOnError = FlutterError.onError;
        FlutterError.onError = (d) {
          if (d.exceptionAsString().contains('overflowed')) return;
    ...
    testWidgets(
      '24. Drawer → Canjes Pendientes navega a la pantalla correcta',
      (tester) async {
        final prevOnError = FlutterError.onError;
        FlutterError.onError = (d) {
          if (d.exceptionAsString().contains('overflowed')) return;
        ...
        await tester.pump(const Duration(milliseconds: 300));
        expect(
          find.text('Canjes Pendientes', skipOffstage: false),
          findsWidgets,
        );
      },
    );
    ...
    testWidgets(
      '25. Drawer → Consejos para Padres navega a la pantalla correcta',
      (tester) async {
        final prevOnError = FlutterError.onError;
        FlutterError.onError = (d) {
          if (d.exceptionAsString().contains('overflowed')) return;
        ...
        await tester.pump(const Duration(milliseconds: 300));
        expect(
          find.text('Consejos para Padres', skipOffstage: false),
          findsWidgets,
        );
      },
    );
    ...
    testWidgets(
      '26. Drawer → Cambiar Color del Panel navega a ColoresPadreScreen',
      (tester) async {
        final prevOnError = FlutterError.onError;
        FlutterError.onError = (d) {
          if (d.exceptionAsString().contains('overflowed')) return;
        ...
        await tester.pump(const Duration(milliseconds: 300));
        expect(
          find.text('Colores del Panel', skipOffstage: false),
          findsOneWidget,
        );
      },
    );
    ...
    testWidgets(
      '27. Drawer → Actividad de Pantalla navega a la pantalla correcta',
      (tester) async {
        final prevOnError = FlutterError.onError;
        FlutterError.onError = (d) {
          if (d.exceptionAsString().contains('overflowed')) return;
        ...
        await tester.pump(const Duration(milliseconds: 300));
        expect(
          find.text('Actividad de Pantalla', skipOffstage: false),
          findsWidgets,
        );
      },
    );

    testWidgets('28. Drawer → Cerrar Sesión navega al LoginScreen', (
      tester,
    ) async {
      final prevOnError = FlutterError.onError;
      FlutterError.onError = (d) {
    ...
    testWidgets(
      '29. Timer del carrusel dispara animateToPage tras 5 segundos',
      (tester) async {
        await cargar(tester);
    ...
    testWidgets(
      '30. Deslizar el PageView dispara onPageChanged y actualiza indicador',
      (tester) async {
        await cargar(tester);
    ...
    testWidgets('31. API 401 con refresh exitoso vuelve a cargar datos', (
      tester,
    ) async {
      var hijosCalled = 0;
      SharedPreferences.setMockInitialValues({
    ...
    testWidgets(
      '32. API 401 con refresh fallido llama _cerrarSesionPorExpiracion',
      (tester) async {
        FlutterBackgroundServicePlatform.instance = _FakeBgService();
        SharedPreferences.setMockInitialValues({
          'user_id': 'padre-uid',
