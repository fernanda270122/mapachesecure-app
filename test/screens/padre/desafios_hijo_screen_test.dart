...
Widget _wrap() => ScreenUtilInit(
  ...
  builder: (_, __) => ChangeNotifierProvider(
    ...
    child: const MaterialApp(home: DesafiosHijoScreen(hijo: _hijoTest)),
  ),
);
...
void main() {
  ...
  group('Pruebas para DesafiosHijoScreen', () {
    ...
    testWidgets('2. Muestra el botón de generar desafíos con IA', (
      tester,
    ) async {
      await tester.pumpWidget(_wrap());
    ...
    testWidgets('5. Muestra el texto del botón de generar con IA', (
      tester,
    ) async {
      await tester.pumpWidget(_wrap());
      await _pumpLoaded(tester);
    ...
    testWidgets('6. Muestra mensaje cuando no hay desafíos asignados', (
      tester,
    ) async {
      await tester.pumpWidget(_wrap());
      await _pumpLoaded(tester);
    ...
    testWidgets('8. Desafíos de tipo fisica se muestran en la sección Física', (
      tester,
    ) async {
      ApiService.testClient = MockClient(
        (req) async => http.Response(
    ...
    testWidgets('9. Desafíos de tipo hogar se muestran en la sección Hogar', (
      tester,
    ) async {
      ApiService.testClient = MockClient(
        (req) async => http.Response(
    ...
    testWidgets('11. Tap en Switch con error de API muestra SnackBar de error', (
      tester,
    ) async {
      ...
      (tester) async {
        ApiService.testClient = MockClient((req) async {
          if (req.url.path.contains('/ia/generar')) {
            return http.Response(
              '{"detail": "Servicio IA no disponible"}',
              503,
            );
          }
    ...
    testWidgets('15. Generar IA exitoso muestra selector de misiones generadas', (
      tester,
    ) async {
      ApiService.testClient = MockClient((req) async {
        if (req.url.path.contains('/ia/generar')) {
    ...
    testWidgets('17. Error al asignar misiones IA muestra SnackBar de error', (
      tester,
    ) async {
      ApiService.testClient = MockClient((req) async {
        if (req.url.path.contains('/ia/generar')) {
    ...
    testWidgets('18. Expandir tarjeta de desafío muestra descripción', (
      tester,
    ) async {
      ApiService.testClient = MockClient(
        (req) async => http.Response(
