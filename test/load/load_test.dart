...
void main() {
  ...
  group('Pruebas de carga — Raccu', () {
    ...
    test(
      '1. 50 usuarios concurrentes cargando 20 desafíos c/u — p95 < 200ms, error < 1%',
      () async {
        const usuarios = 50;
        ...
        debugPrint(
          '── Desafíos: $usuarios usuarios × $desafiosPorUsuario desafíos = $total solicitudes en ${duracionMs}ms',
        );
        debugPrint('   p95        : ${(p95 / 1000).toStringAsFixed(3)}ms');
        debugPrint(
          '   Errores    : $errores / $total (${(tasaError * 100).toStringAsFixed(2)}%)',
        );
        debugPrint(
          '   Throughput : ${opsPorSegundo.toStringAsFixed(0)} ops/s  |  ${opsPorMinuto.toStringAsFixed(0)} ops/min',
        );
        ...
        expect(
          ...
          lessThan(0.01),
          reason:
              'Tasa de error ${(tasaError * 100).toStringAsFixed(2)}% supera el 1%',
        );
      },
    );
    ...
    test(
      '2. 50 guardianes verificando 15 apps bloqueadas c/u — p95 < 200ms, error < 1%',
      () async {
        const usuarios = 50;
        ...
        debugPrint(
          '── Guardián: $usuarios dispositivos × $appsPorUsuario apps = $total verificaciones en ${duracionMs}ms',
        );
        debugPrint('   p95        : ${(p95 / 1000).toStringAsFixed(3)}ms');
        debugPrint(
          '   Errores    : $errores / $total (${(tasaError * 100).toStringAsFixed(2)}%)',
    ...
    test(
      '3. 100 mascotas con puntos distintos — cuántas suben de nivel por segundo',
      () async {
        const usuarios = 200;
        ...
        debugPrint(
          '── Mascotas: $usuarios usuarios concurrentes en ${duracionMs}ms',
        );
        debugPrint(
          '   p95               : ${(p95 / 1000).toStringAsFixed(3)}ms',
        );
        debugPrint(
          '   Errores           : $errores / $usuarios (${(tasaError * 100).toStringAsFixed(2)}%)',
        );
        debugPrint(
          '   Subieron de nivel : $suberonDeNivel / $usuarios usuarios',
        );
        debugPrint(
          '   Evoluciones/s     : ${evolucionesPorSegundo.toStringAsFixed(0)}',
        );
        debugPrint(
          '   Evoluciones/min   : ${evolucionesPorMinuto.toStringAsFixed(0)}',
        );
    ...
    test(
      '4. 100 usuarios cargando su avatar concurrentemente — p95 < 300ms, error < 1%',
      () async {
        const usuarios = 100;
        final ids = [
          'mago',
        ...
        debugPrint(
          '── Avatares: $usuarios usuarios concurrentes en ${duracionMs}ms',
        );
        debugPrint('   p95        : ${(p95 / 1000).toStringAsFixed(3)}ms');
        debugPrint(
          '   Errores    : $errores / $usuarios (${(tasaError * 100).toStringAsFixed(2)}%)',
